...
export const checkQueryParams = (router: NextRouter) => {
	...
	const priceToQueryValue = getQueryParamsOnFirstRender(
		'priceTo',
		router
	) as string

	const partsQueryParams = JSON.parse(
		decodeURIComponent(getQueryParamsOnFirstRender('parts', router) as string)
	)
	const boilersQueryParams = JSON.parse(
		decodeURIComponent(getQueryParamsOnFirstRender('boilers', router) as string)
	)
	...
	const partsQuery = `&parts=${getQueryParamsOnFirstRender('parts', router)}`
