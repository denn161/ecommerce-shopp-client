...
const CheckedFiltersList = ({
	...
	const checkAllManufacturers = () =>
		setManufacturer(items.map((item) => ({ ...item, checked: true })))

	return (
		<>
			<button
				className={cl(styles.filters__btn, {
					[styles.dark]: mode === 'dark',
				})}
				onClick={checkAllManufacturers}
			>
				Выбрать все
			</button>
			<ul
				className={cl(styles.filters__list, {
					[styles.dark]: mode === 'dark',
				})}
			>
				{items.map((item) => (
