...
export const updateTotalPriceProduct = shoppingCard.createEvent<{partId:number,totalPrice:number|undefined}>()
export const updateCountProduct = shoppingCard.createEvent<{partId:number,count:number|undefined}>()
...
const updateTotalPrice =(cartItems:IShoppingCard[],partId:number,totalPrice:number)=>{
...
const updateCount =(cartItems:IShoppingCard[],partId:number,count:number)=>{
	return cartItems.map((item)=>{
		 if(item.partId===partId){
				return {
					 ...item,
...
//  }
 ...
 export const $shoppingCard = shoppingCard.createStore<Array<IShoppingCard>>([])
 ...
 .on(removeProduct,(state,partId)=>[...remove(state,partId)])
  .on(updateTotalPriceProduct,(state,{partId,totalPrice})=>[...updateTotalPrice(state,partId,totalPrice as number)])
	.on(updateCountProduct,(state,{partId,count})=>[...updateCount(state,partId,count as number)])

	export const $totalPrice = shoppingCard.createStore<number>(0)
	.on(setTotalPrice,(_,totalPrice)=>totalPrice)


	export const $disabledCard = shoppingCard.createStore<boolean>(false)
